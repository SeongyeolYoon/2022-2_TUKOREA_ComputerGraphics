...
class Camera {
private:
	glm::vec3 positionVector{ 0.0f,2.0f,5.0f };
	glm::vec3 targetVector{ 0.0f,0.0f,0.0f };
...
GLvoid Keyboard(unsigned char key, int x, int y)
{
	...
	case 'y':
		...
		pCamera->isOnRotation() = !pCamera->isOnRotation();
		if(true == pCamera->isOnRotation() )
			pCamera->getRotationDirection() = Direction::Positive;
		else
			pCamera->getRotationDirection() = Direction::Idle;
		break;
	case 'Y':
		pCamera->isOnRotation() = !pCamera->isOnRotation();
		if (true == pCamera->isOnRotation())
			pCamera->getRotationDirection() = Direction::Negative;
		else
			pCamera->getRotationDirection() = Direction::Idle;
		break;
	case 'r':
...
GLvoid TimerFunc(int value)
{
	...
	if (pCamera->isOnRotation()) {

		switch (pCamera->getRotationDirection())
		{
