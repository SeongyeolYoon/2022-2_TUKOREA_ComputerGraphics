...
Orbit::Orbit(const GLfloat& degreeIn)
{
	degree = degreeIn;
}

void Orbit::initialize(const char* objName)
{
	scaleVector = glm::vec3{ 0.01f,0.01f,0.01f };
	positionVector.x = radian * glm::radians(cosf(degree));
	positionVector.y = 0.0;
	positionVector.z = radian * glm::radians(sinf(degree));
	colorVector = glm::vec3(0.0f, 1.0f, 0.0f);

	readObj(objName);
}

void Orbit::readObj(const char* objName)
{
...
void Orbit::render(const glm::mat4& parent)
{
