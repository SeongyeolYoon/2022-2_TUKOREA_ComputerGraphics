...
void Light::initialize(const char* fileName)
{
	positionVector = glm::vec3(-4.0f, 0.0f, 0.0f);
	scaleVector = glm::vec3(0.25f, 0.25f, 0.25f);
	readObj(fileName);
}
...
const GLfloat& Light::getMoveAmount() const { return moveAmount; }
...
void Light::update()
{
	...
	case State::REVOLVE:
	{
		...
		positionVector.x = radian * glm::radians(cosf(revolutionDegree));
		positionVector.y = 1.0;
		positionVector.z = radian * glm::radians(sinf(revolutionDegree));
		std::cout << "revolutionDegree: " << revolutionDegree << "\n";
		break;
	}
	default:
		break;
