...
void Planet::initialize(const char* objName)
{
	readObj(objName);
	scaleVector = glm::vec3(0.5f, 0.5f, 0.5f);
}

void Planet::readObj(const char* objName)
{
...
void Planet::render()
{
	...
	parentMatrix = glm::translate(parentMatrix, glm::vec3(0.0f, 2.0f, 0.0f));
	revolutionMatrix = glm::rotate(revolutionMatrix, glm::radians(revolutionDegree), orbitDirection);
...
void Planet::setColor(const glm::vec3& colorIn)
{
	color = colorIn;
}

void Planet::setOrbitDirection(const glm::vec3& direction)
{
	orbitDirection = direction;
}

void Planet::setPositionVector(const glm::vec3& position)
{
