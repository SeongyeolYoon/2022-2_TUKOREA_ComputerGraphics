...
void Snow::initialize(const char* objName)
{
	readObj(objName);
	fallSpeed = randomFallSpeed(mersenne);
	positionVector = glm::vec3(randomXPos(mersenne), 8.0f, randomZPos(mersenne));
	scaleVector = glm::vec3(0.05f, 0.05f, 0.05f);
}

void Snow::readObj(const char* objName)
{
...
void Snow::update()
{
	...
	case State::IDLE:
		break;
	case State::FALL:
		positionVector.y -= fallSpeed;
		if (positionVector.y < 0.0f) {
			positionVector.y = 3.0f;
		}
		break;
	default:
		break;
...
void Snow::render()
{
	...
	translationMatrix = glm::translate(transformMatrix, positionVector);
...
void Snow::setState(State state)
{
	curState = state;
}