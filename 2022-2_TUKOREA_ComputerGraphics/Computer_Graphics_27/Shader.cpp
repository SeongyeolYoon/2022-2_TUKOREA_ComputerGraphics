...
void Shader::glGenerate(std::vector<Pillar*>& pPillar, std::vector<Snow*>& pSnow)
{
	// Floor
...
void Shader::initTexture(std::vector<Pillar*>& pPillar, std::vector<Snow*>& pSnow)
{
	// Floor
	{
	...
	// Pyramid
	{
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		stbi_set_flip_vertically_on_load(true);
		unsigned char* data = stbi_load("pyramid.png", &imageWidth, &imageHeight, &numberOfChannel, 0);
		glTexImage2D(GL_TEXTURE_2D, 0, 3, imageWidth, imageHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		stbi_image_free(data);
	}
}
...
void Shader::initBuffer(std::vector<Pillar*>& pPillar, std::vector<Snow*>& pSnow)
{
	// Floor
	{
	...
	// Pyramid
	{
		...
		glBindBuffer(GL_ARRAY_BUFFER, Pyramid::getInstance()->colorVbo);
		glBufferData(GL_ARRAY_BUFFER, Pyramid::getInstance()->colors.size() * sizeof(glm::vec3), Pyramid::getInstance()->colors.data(), GL_STATIC_DRAW);
		GLint cAttribute = glGetAttribLocation(shaderProgram, "vColor");
		glVertexAttribPointer(cAttribute, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), 0);
		glEnableVertexAttribArray(cAttribute);
	}
	...
	for (size_t i = 0; i < PILLARS; ++i) {
		...
		glBufferData(GL_ARRAY_BUFFER, pPillar[i]->colors.size() * sizeof(glm::vec4), pPillar[i]->colors.data(), GL_STATIC_DRAW);
		GLint cAttribute = glGetAttribLocation(shaderProgram, "vColor");
	...
	for (size_t i = 0; i < SNOWS; ++i) {
		...
		glBufferData(GL_ARRAY_BUFFER, pSnow[i]->colors.size() * sizeof(glm::vec4), pSnow[i]->colors.data(), GL_STATIC_DRAW);
		GLint cAttribute = glGetAttribLocation(shaderProgram, "vColor");
		glVertexAttribPointer(cAttribute, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(float), 0);
		glEnableVertexAttribArray(cAttribute);
	}
