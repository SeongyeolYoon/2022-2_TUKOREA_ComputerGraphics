...
void Pillar::initialize(const char* objName)
{
	readObj(objName);
	scaleVector = glm::vec3{ 0.3f, 3.0f, 0.3f};
	positionVector = glm::vec3{ randomXPos(mersenne), 0.0f, randomZPos(mersenne) };
}

void Pillar::readObj(const char* objName)
{
	...
	for (size_t i = 0; i < verticies.size(); ++i) {
		colors.push_back(glm::vec4(0.5f, 0.5f, 0.5f, 0.5f));
	}
}
